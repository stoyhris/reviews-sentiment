def main():
    ...
    def no_stops(file): 
        ...
        for line in file: 
            words = line[:-1]
            label = line[-1]
            cleaned_line = [word for word in words if word.lower() not in stopwords] # empty strings??
            if not cleaned_line: 
                cleaned_line.append('')
            #print(f"OG line: {line}")
            #print(f"Clean Line: {cleaned_line}")
            clean_file.append(cleaned_line + [label])
        #print(f"Cleaned file: {file}")
        return clean_file 

    mylist_neg = tokenize(mylist_neg)
    mylist_pos = tokenize(mylist_pos)

    mylist_neg = no_specs(mylist_neg)
    mylist_pos = no_specs(mylist_pos)

    ### LABELS AND MERGE : 0 if negative, 1 if positive###
    mylist_neg = [line +[0] for line in mylist_neg]
    mylist_pos = [line + [1] for line in mylist_pos]
